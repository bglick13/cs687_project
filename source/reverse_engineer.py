...
if __name__ == '__main__':
    ...
    for episode in tqdm(candidate_data.episodes):
        for i in range(len(episode.states)):
            s = episode.states[i]
            a = episode.actions[i]
            pb_sa = episode.pb_sas[i]
            agent.policy[s, a] += pb_sa
    agent.policy /= 500000
    print('Policy:')
    print(np.exp(agent.policy) / np.exp(agent.policy).sum(1)[:, None])
    pdis, left_side, did_pass = agent.safety_test(safety_data, False, agent.policy)
    print(f'PDIS: {pdis}, Left Side: {left_side}, Did Pass: {did_pass}')
